...
public:
    ...
    cv::Mat nonZeroMean(cv::Mat raw_image, int mask_size)
    {
        ...
        cv::Mat mean_image = cv::Mat(h, w, CV_8UC1);
        ...
        for (int i = 0; i < h; i++)
        {
            for (int j = 0; j < w; j++)
            {
                int num = 0;
                int sum = 0;
                //ROS_INFO("i:%d, j:%d, raw:%d", i, j, raw_ptr[i * w + j]);
                ...
                for (int x = -mf; x < mo; x++)
                {
                    for (int y = -mf; y < mo; y++)
                    {
                        ...
                        {
                            //ROS_INFO("x:%d, y:%d, raw:%d", x, y, raw_ptr[(i + x) * wf + (j + y)]);
                            int px = raw_ptr[(i + x) * w + (j + y)];
                ...
                mean_ptr[i * w + j] = num > 0 ? sum / num : 0;
...
int main(int argc, char** argv)
{
    ...
    int mask_median; nh.param("mask_median", mask_median, 9);
